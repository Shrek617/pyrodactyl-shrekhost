...
function TelegramLoginWidget() {
    ...
    useEffect(() => {
        // Load Telegram widget script in background for Telegram.Login.auth() API
    ...
    const handleClick = () => {
        ...
        (window as any).Telegram?.Login?.auth(
            ...
            (data: any) => {
                ...
                // Redirect to our callback with auth data as query params
                const params = new URLSearchParams();
                Object.keys(data).forEach((key) => params.set(key, data[key]));
                window.location.href = `/auth/login/oauth/telegram/callback?${params.toString()}`;
            }
        );
    };
    ...
    return (
        <button
            ...
            className='flex items-center justify-center w-full px-4 py-2.5 text-sm font-bold text-white transition-all duration-200 bg-[#2481cc] rounded-full hover:bg-[#1d6fa5] hover:shadow-lg cursor-pointer disabled:opacity-50 disabled:cursor-wait'
        >
        ...
        >
            {({ isSubmitting }) => (
                <LoginFormContainer className={`w-full flex`}>
                    ...
                    {(window as any).SiteConfiguration?.telegram?.enabled && (
                        <>
